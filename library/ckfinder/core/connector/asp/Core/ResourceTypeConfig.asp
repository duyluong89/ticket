...
class CKFinder_Connector_Core_ResourceTypeConfig
	...
	Private function checkSingleExtension(extension)
		If deniedExtensions<>"" Then
			If oCKFinder_Factory.RegExp.MatchesPattern( "^" & deniedExtensions & "$", extension ) Then
		...
		If allowedExtensions<>"" Then
			If Not oCKFinder_Factory.RegExp.MatchesPattern( "^" & allowedExtensions & "$", extension ) Then
				checkSingleExtension = False
				Exit function
			End If
		End If
